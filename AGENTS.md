...
| `AfiliacionService` | `POST afiliacion` | `PUT afiliacion`        | `GET afiliacion/find_by/id_persona/:id`, `GET static_catalog/estado` (returns `Estado[]`)                                                                                                                                                                                           |
...
`Afiliacion` body: `{ idAfiliacion?, idPersona, idEstado, fechaInicio, fechaFin, vitalicia, deleted }`. `idEstado` is a mandatory FK to the `Estado` catalog (`{ idEstado, abreviado, nombre }` from `GET static_catalog/estado`). `fechaInicio` / `fechaFin` are sent as `yyyy-MM-dd` but may be returned as epoch-millis numbers. The afiliación no longer carries image ids — its images are `Imagen` rows linked via `idAfiliacion`.
